#!/bin/bash
# Script d'initialisation de cron pour l'entraÃ®nement planifiÃ©

# CrÃ©er le rÃ©pertoire pour les logs
mkdir -p /app/logs

# Copier le script cron
cp /app/docker/api/cron_training.sh /app/cron_training.sh
chmod +x /app/cron_training.sh

# Configurer le crontab (tous les jours Ã  2h du matin)
# Configurer le crontab (tous les jours à 2h du matin)
echo "0 2 * * * /app/cron_training.sh >> /app/logs/cron_training.log 2>&1" | crontab -

# DÃ©marrer cron en arriÃ¨re-plan
cron

echo "Cron configurÃ© pour l'entraÃ®nement planifiÃ© (tous les jours Ã  2h)"
